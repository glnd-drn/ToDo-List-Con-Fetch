...
const Home = () => {
	...
	useEffect(() => {
		...
		const dataFetch = async () => {
			try {
				const response = await fetch('https://playground.4geeks.com/apis/fake/todos/user/glen', {
					method: "GET",
					headers: {
				...
				if (!response.ok) {
					...
					const putResponse = await fetch('https://playground.4geeks.com/apis/fake/todos/user/glen', {
						method: "PUT",
						body: JSON.stringify(["Lista"]),
						headers: {
							"Content-Type": "application/json"
